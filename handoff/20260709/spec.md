...
### feat: allow events to have empty title

<https://github.com/SwitchbackTech/compass-calendar/issues/1871>

## Done - Ignore the below, we already did these

## fix(web): adjust logout menu and cmd
...
## fix(web): make copy in loader and elsewhere more human
...
## fix: when quickly nudging, the event lags behind
...
## style(week): adjust the now line to only appear on the current day's column
...
## style(forms): consolidate form styling
...
### feat(day): make the default task list width 600

- compass.day.task-list-width

### refactor(web): simplify floating-ui implementations
...
### fix(day): user cannot migrate task when thumb icon is focused
