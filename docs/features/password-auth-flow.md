...
- The backend accepts only the configured Compass Google callback URL as the
  ...
  from backend `ENV.FRONTEND_URL`, loaded from `urls.frontend`, plus `/auth/google/callback`.
...
The host/origin portion is taken from backend config (`urls.frontend`), and the
route is always `/day`.
...
- Reset password links always target the `/day` route and require a valid `urls.frontend` in backend config.
