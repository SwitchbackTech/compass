...
2. register Redux listener middleware during store construction
...
| Async sequences and persistence orchestration | Redux Toolkit listener middleware + TanStack Query | `packages/web/src/ducks/events/listeners/event.operations.ts` |
...
2. Redux listener middleware handles the async side effect
3. the selected repository writes locally or remotely
4. the listener updates the Redux event state
...
- when debugging, inspect the action, listener, repository, and store layer together
...
- Event refresh issue: read SSE hooks, sync slice, event listeners.
