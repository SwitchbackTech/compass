...
APPROVAL: agent-automerge + merge-guard; bun run verify PASS + required checks
...
| Concern | File |
| --- | --- |
| Trigger, concurrency, kill switch | `.github/workflows/agent-loop.yml` |
...
| Post-deploy (smoke + annotate + launch-next flag) | `.github/scripts/agent-loop-postdeploy.sh` |
...
1. **Pick next WP** (`.github/scripts/agent-loop-next.sh`): walk
   `AGENT_LOOP_MILESTONES` in order. Take the lowest open issue in the
   first milestone that has one, skipping `agent-loop-running` /
   `agent-loop-needs-human` and issues with an open PR with `Fixes #<n>`.
   If any issue has a fresh running label, idle (concurrency 1). Labels
   older than 3 hours are treated as abandoned and cleared.
...
3. **Agent** follows that prompt: implement the WP, run `bun run verify`,
   ...
   boundary is `allow`.
4. **Merge guard** (`.github/scripts/agent-loop-merge-guard.sh`): wait for
   CI, then squash-merge when size is under the rails and no sensitive
   path is in the diff, unless a per-milestone allowlist re-allows that
   prefix. Otherwise add `agent-loop-needs-human` and stop.
5. **Release on main** deploys staging (code paths only; docs-only merges
   skip deploy). The 15-minute cron is the watchdog for docs-only WPs.
6. **Post-deploy** (`agent-loop.yml` `workflow_run`): smoke staging, strip
   `agent-loop-running`, launch the next WP. `Fixes #<n>` closes the issue.

`concurrency.group: agent-loop` with `cancel-in-progress: false` means a
second trigger **waits**, it does not cancel the first run. Do not change
this model here; that is Providers L WP-02.
...
| Second trigger while in flight | Queued behind `concurrency.group: agent-loop`; first run not cancelled |
