...
When encountering an invalid domain name error, this is because the URL you provided in `supertokens.uri` in your active Compass config is incorrect. For local development that is usually `packages/backend/compass.yaml`. This could be caused by prematurely finishing the setup of your Supertokens instance.
