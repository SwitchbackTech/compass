...
export class MapGCalEvent {
  ...
  static toEvent(
    ...
    const { id, recurringEventId } = _event;
    ...
    if (isBaseGCalEvent(gEvent)) Object.assign(event, { recurrence: { rule } });
    if (isInstanceGCalEvent(gEvent)) {
      Object.assign(event, {
        recurrence: RecurrenceInstanceSchema.parse(overrides.recurrence),
      });
    }
  ...
  static fromEvent<GEvent extends gSchema$Event = gSchema$Event>(
    event: Schema_Event,
    { status = EventStatus.CONFIRMED }: Pick<gSchema$Event, "status"> = {},
  ): GEvent {
    ...
    if (hasRecurrenceRule) gcalEvent.recurrence = rule;
