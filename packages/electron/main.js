...
const port = 9080; // Hardcoded; needs to match webpack.development.js
...
async function createWindow() {
  ...
  win = new BrowserWindow({
    ...
    webPreferences: {
      // devTools: isDev, always allow for prod debugging
  ...
  win.webContents.on("did-finish-load", () => {
    win.setTitle(`Compass Calendar (v${app.getVersion()})`);
  });
  ...
  if (isDev) {
    ...
    win.webContents.once("dom-ready", async () => {
      await installExtension([REDUX_DEVTOOLS, REACT_DEVELOPER_TOOLS])
        ...
        .catch((err) =>
          console.log("An error occurred while install extensions: ", err)
        )
        .finally(() => {
...
app.on("web-contents-created", (event, contents) => {
  ...
  contents.on("will-redirect", (contentsEvent, navigationUrl) => {
    const parsedUrl = new URL(navigationUrl);
    const validOrigins = ["https://www.google.com/"];
