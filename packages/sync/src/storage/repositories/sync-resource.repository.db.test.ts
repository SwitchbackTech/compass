...
describe("SyncResourceRepository", () => {
  ...
  it("reads a row a newer build stamped with an unknown field", async () => {
    // A rolling deploy runs the old and new builds together: the new build adds
    // a field and stamps it on a row, then the old build reads that row. A
    // strict read schema rejected the unknown key and broke GET /connections.
    const resource = await repo.ensure(upsert());
    await db
