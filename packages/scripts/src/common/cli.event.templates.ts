...
const templates: EventTemplate[] = [
  ...
  // 1-day all-day event
  {
    ...
    end: (now) => getDateTime(now, { days: 7 }),
  },
  // 1-hour recurring event (Mon, Wed, Fri)
  {
    title: "🏃 Run",
    description:
      "A recurring 1-hour event from 11am to 12pm every Monday, Wednesday, and Friday",
    isSomeday: false,
    isAllDay: false,
    count: 1,
    start: (now) => getDateTime(now, { days: 1, hours: 11, minutes: 0 }),
    end: (now) => getDateTime(now, { days: 1, hours: 12, minutes: 0 }),
    recurrence: {
      rule: ["RRULE:FREQ=WEEKLY;COUNT=3;INTERVAL=1;BYDAY=MO,WE,FR"],
    },
  },
  ...
  },
  {
    ...
    isAllDay: false,
    count: 1,
    start: (now) => getDateTime(now, { days: 2, hours: 14, minutes: 30 }),
    end: (now) => getDateTime(now, { days: 2, hours: 15, minutes: 15 }),
    recurrence: {
      rule: ["RRULE:FREQ=WEEKLY;COUNT=3;INTERVAL=1;BYDAY=TU,WE,SA"],
    },
  },
  {
  ...
  },
  {
    ...
    isAllDay: false,
    count: 1,
    start: (now) => getDateTime(now, { days: 1, hours: 16, minutes: 0 }),
    end: (now) => getDateTime(now, { days: 1, hours: 18, minutes: 0 }),
    recurrence: {
      rule: ["FREQ=WEEKLY;BYDAY=MO,TU,WE;COUNT=3"],
    },
  },
  {
    title: "🎬 Watch play",
