...
const deckOf = (
  items: ReturnType<typeof createWeekTimedEventLayout>,
  id: string,
) => items.find((item) => item.event._id === id)?.deckLayout ?? null;

describe("createWeekTimedEventLayout", () => {
  it("returns render-only deck layout for same-day overlapping events", () => {
    ...
    const laid = createWeekTimedEventLayout(events);
...
describe("applyWeekTimedDeckPosition", () => {
  ...
  it("keeps narrow-column decks inside the usable column", () => {
    ...
    const back = applyWeekTimedDeckPosition(narrowBase, {
      order: 0,
      groupSize: 2,
    });
