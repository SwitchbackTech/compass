...
describe("taskIdToUnderscoreIdMigration", () => {
  it("maps legacy id to _id and re-saves tasks", async () => {
    ...
    store.getAllTasks.mockResolvedValue(legacyTasks as never);

    await taskIdToUnderscoreIdMigration.migrate(store);
  ...
  it("skips tasks with neither id nor _id", async () => {
    ...
    expect(store.putTasks.mock.calls[0][1]).toHaveLength(1);
  });
  ...
  it("clears dateKeys that contain only invalid tasks (neither id nor _id)", async () => {
    ...
    store.getAllTasks.mockResolvedValue([invalidTask] as never);

    await taskIdToUnderscoreIdMigration.migrate(store);

    expect(store.putTasks).toHaveBeenCalledWith("2025-01-16", []);
  });
