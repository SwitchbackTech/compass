import { Schema_DraftEvent } from "../schemas/events/draft.event.schemas";
import { Schema_WebEvent } from "../schemas/events/web.event.schemas";

/**
 * Parser class for determining if an event has been modified (is dirty)
 */
export class DraftParser {
  private readonly curr: Schema_DraftEvent;
  private readonly orig: Schema_WebEvent;

  constructor(curr: Schema_DraftEvent, orig: Schema_WebEvent) {
    this.curr = curr;
    this.orig = orig;
  }
...
export const isDraftDirty = (
  curr: Schema_DraftEvent,
  orig: Schema_WebEvent,
): boolean => {
  const parser = new DraftParser(curr, orig);
