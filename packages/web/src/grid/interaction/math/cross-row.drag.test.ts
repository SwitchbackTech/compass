...
describe("resolveDragRow", () => {
  it("resolves to the all-day row inside its rect and the timed grid below it", () => {
    ...
    expect(resolveDragRow({ ...rows, pointerY: 40 })).toBe("allDay");
    expect(resolveDragRow({ ...rows, pointerY: 20 })).toBe("allDay");
    expect(resolveDragRow({ ...rows, pointerY: 60 })).toBe("allDay");
    expect(resolveDragRow({ ...rows, pointerY: 61 })).toBe("timed");
    expect(resolveDragRow({ ...rows, pointerY: 800 })).toBe("timed");
  });
  ...
  it("is pure in the pointer, so the engine's pointerup rerun reproduces the row", () => {
    ...
    expect(resolveDragRow(input)).toBe(resolveDragRow(input));
  });
});

describe("getDragRowLayouts", () => {
  it("reads the primary cache as the drag's own row and crossRow as the other", () => {
    const paired = { ...allDayLayout, crossRow: timedLayout };

    expect(getDragRowLayouts(paired, "allDay")).toEqual({
      allDay: paired,
      timed: timedLayout,
    });
    expect(
      getDragRowLayouts({ ...timedLayout, crossRow: allDayLayout }, "timed"),
    ).toEqual({
      allDay: allDayLayout,
  ...
  it("reports the other row as absent when it was never cached", () => {
    expect(getDragRowLayouts(allDayLayout, "allDay").timed).toBeNull();
  });
});
