...
class UserService {
  ...
  stopGoogleCalendarSync = async (user: ObjectId): Promise<void> => {
    await eventService.deleteByIntegration(CalendarProvider.GOOGLE, user);
    await syncService.stopWatches(user);
  ...
  startGoogleCalendarSync = async (user: ObjectId): Promise<void> => {
    ...
    await syncService.importFull(gcal, calendars);
  };

  restartGoogleCalendarSync = async (userId: ObjectId) => {
