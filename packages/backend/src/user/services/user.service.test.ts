...
import * as syncInit from "@backend/sync/services/init/sync.init";
...
describe("UserService", () => {
  ...
  describe("startGoogleCalendarSync", () => {
    ...
    it("starts only selected Google calendar syncs", async () => {
      const { getCalendarsToSync } = syncInit;

      // remove after full sync support is active
      jest
        .spyOn(syncInit, "getCalendarsToSync")
        .mockImplementation((gcal) => getCalendarsToSync(gcal, false));
