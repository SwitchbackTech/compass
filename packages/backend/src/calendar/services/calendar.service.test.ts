...
describe("CalendarService", () => {
  ...
  describe("create", () => {
    it("creates a google calendar entry", async () => {
      const user = await AuthDriver.googleSignup();
      const gcal = await getGcalClient(user._id);
      const { calendars } = await getCalendarsToSync(gcal);
  ...
  describe("toggleSelection", () => {
    it("updates selection state for given calendars", async () => {
      ...
      const calendarList = await calendarService.getAllByUser(userId);
      ...
      const updatedCalendarList = await calendarService.getAllByUser(userId);
