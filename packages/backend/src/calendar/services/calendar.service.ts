...
class CalendarService {
  ...
  async getByUser(user: ObjectId, _id: ObjectId) {
    const calendar = await mongoService.calendar.findOne({ user, _id });
   ...
   */
  getAllByUser = async (user: ObjectId, provider?: CalendarProvider) => {
    const filter: Filter<Schema_Calendar> = { user };

    if (provider) Object.assign(filter, { "metadata.provider": provider });

    return await mongoService.calendar.find(filter).toArray();
  };

  getAllByUserAndProvider = async (
    user: ObjectId,
    provider: CalendarProvider,
  ) => {
    return await mongoService.calendar
      .find({ user, metadata: { provider } })
      .toArray();
  };
