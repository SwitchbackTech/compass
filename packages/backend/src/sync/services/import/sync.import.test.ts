import { generateGcalEvents as mockGcalEvents } from "@backend/__tests__/factories/event.factory";
...
const NEXT_SYNC_TOKEN = "final-sync-token";
...
describe("SyncImport", () => {
  ...
  describe("Full import", () => {
    it("should include regular and recurring events and skip cancelled events", async () => {
      ...
      expect(nextSyncToken).toBe(NEXT_SYNC_TOKEN);
    });
  });
});
