...
describe("googleCalendarSyncService", () => {
  ...
  describe("importLatestGoogleCalendarChanges", () => {
    it("skips a completed incremental import without publishing status", async () => {
      ...
      const createSyncImportSpy = spyOn(
        syncImportService,
        "createSyncImport",
      );
  ...
  describe("initializeGoogleCalendarSync", () => {
    ...
    it("does not abort other calendars when one calendar's import fails, and still starts watches for primary", async () => {
      ...
      spyOn(syncImportService, "createSyncImport").mockResolvedValue({
        ...
        ReturnType<typeof syncImportService.createSyncImport>
      >);
      const startWatchesSpy = spyOn(googleWatchService, "startGoogleWatches")
  ...
  describe("repairGoogleCalendarSync", () => {
    it("forces sync setup when import is completed", async () => {
      ...
      const stopSpy = spyOn(userService, "stopGoogleCalendarSync")
        .mockResolvedValue();
      const startSpy = spyOn(googleCalendarSyncService, "initializeGoogleCalendarSync")
        .mockResolvedValue({
          eventsCount: 0,
          calendarsCount: 0,
          failedCalendars: [],
