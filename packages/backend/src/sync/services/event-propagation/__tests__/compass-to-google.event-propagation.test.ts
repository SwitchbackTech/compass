...
describe("CompassToGoogleEventPropagation.getNotificationType", () => {
  it("returns EVENT_CHANGED for non-SOMEDAY transitions", () => {
    ...
    expect(
      CompassToGoogleEventPropagation["getNotificationType"](transition),
    ).toEqual([EVENT_CHANGED, EVENT_CHANGED]);
  });
  ...
  it("returns SOMEDAY_EVENT_CHANGED for SOMEDAY transitions", () => {
    ...
    expect(
      CompassToGoogleEventPropagation["getNotificationType"](transition),
    ).toEqual([SOMEDAY_EVENT_CHANGED, SOMEDAY_EVENT_CHANGED]);
  });
  ...
  it("returns mixed notifications for mixed transitions", () => {
    ...
    expect(
      CompassToGoogleEventPropagation["getNotificationType"](transition),
    ).toEqual([SOMEDAY_EVENT_CHANGED, EVENT_CHANGED]);
  });
});

describe("CompassToGoogleEventPropagation.notifyClients", () => {
  beforeEach(() => {
