...
describe.each([
...
])(`CompassToGoogleEventPropagation - $calendarProvider calendar: ${RecurringEventUpdateScope.ALL_EVENTS}`, ({
  calendarProvider,
}) => {
    ...
    describe("Calendar: ", () => {
            ...
            case CalendarProvider.GOOGLE: {
              ...
              expect(gcalEvent).toEqual(
                expect.objectContaining({
                  id: updatedBase!.gEventId,
                  start: expect.objectContaining({
                    dateTime: expect.not.stringMatching(updatedPayload.endDate),
                  }),
                }),
              );
      ...
      describe("Transition Edits: ", () => {
        ...
        it("should update the recurrence(rule) field of an event - to base calendar event", async () => {
          ...
          switch (calendarProvider) {
            case CalendarProvider.GOOGLE:
              await testCompassSeriesInGcal(updatedBaseEvent, updatedInstances);
              break;
          }
        });
  ...
  describe("Delete - Instance Event: ", () => {
    it("should delete a recurring event series", async () => {
      ...
      const updateChanges = await CompassToGoogleEventPropagation.processEvents(
        [
          {
            payload: updatedPayload as CompassAllEvents["payload"],
            applyTo: RecurringEventUpdateScope.ALL_EVENTS,
        ...
        case CalendarProvider.GOOGLE:
          ...
          await Promise.all(
            instances.map((instance) =>
              ...
              ).rejects.toThrow(`Event with id ${instance.gEventId} not found`),
            ),
          );
