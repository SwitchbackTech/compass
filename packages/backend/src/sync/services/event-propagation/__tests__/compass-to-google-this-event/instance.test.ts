...
describe.each([
...
])(`CompassToGoogleEventPropagation - $calendarProvider calendar: ${RecurringEventUpdateScope.THIS_EVENT} - Instance Event: `, ({
  calendarProvider,
}) => {
    ...
    describe("Calendar: ", () => {
        ...
        it("should not update the recurrence field of an instance event", async () => {
          ...
          switch (calendarProvider) {
            case CalendarProvider.GOOGLE: {
              ...
              expect(event).toHaveProperty("gEventId");
              // check that event exist in gcal
              const gcalEvent = await _getGcal(user, updatedInstance.gEventId!);
      ...
      describe("Transition Edits: ", () => {
        it("should not update the isSomeday(to true) field of an event - to instance someday event", async () => {
          ...
          const updateChanges = CompassToGoogleEventPropagation.processEvents([
            {
              payload: updatedPayload as CompassThisEvent["payload"],
              applyTo: RecurringEventUpdateScope.THIS_EVENT,
              status: CompassEventStatus.CONFIRMED,
  ...
  describe("Delete - Instance Event: ", () => {
    it("should delete a calendar event", async () => {
      ...
      const deleteChanges = await CompassToGoogleEventPropagation.processEvents(
        [
          {
            payload: {
              ...deletedInstance,
      ...
      const baseEvent = await eventService.readById(user, event._id.toString());
