...
const _getNextStart = (rule: string, startDate: string, endDate: string) => {
  switch (rule) {
    case RRULE.WEEK:
      return _getFirstDayOfNextWeek(startDate);
      break;
    case RRULE.MONTH:
...
const _getNextMonth = (target: string) => {
  const date = dayjs(target, YEAR_MONTH_DAY_FORMAT).startOf("d");

  const firstOfNextMonth = date.add(1, "month").startOf("m");

  return firstOfNextMonth;
};

// could be Sunday or Monday - depending on locale
const _getFirstDayOfNextWeek = (startDate: string) => {
  const date = dayjs(startDate, YEAR_MONTH_DAY_FORMAT).startOf("d");

  return date.add(1, "week").weekday(0);
};
