    ...
    return CompassSyncProcessor.processEvents(
      [weeklyRepeat, monthlyRepeat, defaultWeekly].map((e) => {
        ...
        return {
          eventId,
          userId,
          payload: { ...e, _id: eventId } as Event_Core & { recurrence: never },
...
export const _createCompassEvent = async (
  ...
  const calData = !isSomeday ? MapEvent.toProviderData(_event, provider) : {};
  const event = Object.assign(_event, calData, { updatedAt });
...
export const _updateSingleCompassEvent = async (
  _event: WithId<Omit<Schema_Event, "_id">> & { user: string },
  newDocument?: boolean,
  session?: ClientSession,
): Promise<WithCompassId<Omit<Schema_Event, "_id">>> => {
  ...
  let cEvent: WithId<Omit<Schema_Event, "_id">> | null;
  ...
  if (newDocument) {
    ...
    const [newEvent] = await Promise.all([
      mongoService.event
        ...
        .then(({ insertedId }) => ({ _id: insertedId, ...payload })),
      _deleteSingleCompassEvent(event, session),
    ]);

    cEvent = newEvent;
  } else {
    cEvent = await mongoService.event.findOneAndReplace(
      { _id: event._id, user: event.user },
      event,
...
export const _updateCompassSeries = async (
  ...
  const updatedAt = new Date();
  const update = Object.assign(payload, { updatedAt });

  await mongoService.event.updateMany(
    {
      $or: [
        { _id: baseId, user },
        { "recurrence.eventId": baseId, user },
      ],
    },
    { $set: update },
    { session },
...
export const _createGcalEvent = async (
  userId: string,
