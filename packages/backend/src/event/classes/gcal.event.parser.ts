...
export class GcalEventParser {
  ...
  constructor(calendar: Schema_Calendar, event: WithGcalId<gSchema$Event>) {
  ...
  async init(session?: ClientSession): Promise<void> {
    ...
    this.#status =
      status == "cancelled" ? EventStatus.CANCELLED : EventStatus.CONFIRMED;
    ...
    this.#isCompassStandalone = this.#dbEvent
      ? isRegularEvent(this.#dbEvent)
      : false;
    ...
    this.#transition = [
      ...
      `${this.#getCategory()}_${this.#status}`,
    ];
  }
