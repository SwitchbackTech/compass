#!/usr/bin/env bash
# Deterministic re-check, verification wait, and squash-merge for an agent
# loop PR.
...
check_and_merge() {
  ...
  if ! gh pr checks "$pr_number" --repo "$REPO" --watch --fail-fast; then
    notify "Agent-loop PR #${pr_number} failed CI after merge-guard verification: https://github.com/${REPO}/pull/${pr_number}"
    return 0
  fi

  if ! gh pr merge "$pr_number" --repo "$REPO" --squash --delete-branch; then
    notify "Agent-loop PR #${pr_number} passed verification but direct squash-merge failed: https://github.com/${REPO}/pull/${pr_number}"
    return 0
  fi
  printf 'squash-merged agent PR #%s\n' "$pr_number"
}
